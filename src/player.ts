...
export class Player extends Actor {
  ...
  private _maestroSpritesheet = SpriteSheet.fromImageSourceWithSourceViews({
    image: Resources.MaestroSpritesheet,
    sourceViews: [
  ...
  constructor() {
    ...
    this._maestroSpritesheet.sprites.forEach((sprite) => {
  ...
  override onInitialize() {
    ...
    this.graphics.use(
      Animation.fromSpriteSheetCoordinates({
        spriteSheet: this._maestroSpritesheet,
        frameCoordinates: [
          { x: 0, y: 0 },
          { x: 1, y: 0 },
          { x: 2, y: 0 },
  ...
  override onPreUpdate(engine: Engine, elapsedMs: number): void {
    ...
    let lineEnd = mousePos?.sub(this.pos);
